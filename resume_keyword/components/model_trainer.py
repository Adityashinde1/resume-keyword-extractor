...
class ModelTrainer:
    ...
    def get_annotations_from_s3(self, bucket_name: str, local_folder_path: str, bucket_folder_name: str):
        try:
            logger.info("Entered the get_annotations_from_s3 method of Model trainer class")
            command: str = (
            f"aws s3 sync s3://{bucket_name}/{bucket_folder_name}/ {local_folder_path} "
        )

            os.system(command)
            logger.info("Exited the get_annotations_from_s3 method of Model trainer class")
    ...
    def create_spacy_format_data(self, data: json, docbin_obj: object, output_file_path: str, nlp_model: object):
        try:
            logger.info("Entered the create_spacy_format_data method of Model trainer class")
            ...
            logger.info("Exited the create_spacy_format_data method of Model trainer class")
    ...
    def initiate_model_trainer(self):
        try:
            ...
            self.s3.download_file(bucket_name=BUCKET_NAME, output_file_path=self.model_trainer_config.TRAIN_ANNOTAIONS_PATH, key=TRAIN_ANNOTATION_FILE_NAME)
            logger.info("Downloaded train annotation file from s3 bucket")

            self.s3.download_file(bucket_name=BUCKET_NAME, output_file_path=self.model_trainer_config.TEST_ANNOTAIONS_PATH, key=TEST_ANNOTATION_FILE_NAME)
            logger.info("Downloaded test annotation file from s3 bucket")

            train_annotations_local_path = os.path.join(from_root(), TRAIN_ANNOTATION_FILE_NAME)
            test_annotations_local_path = os.path.join(from_root(), TEST_ANNOTATION_FILE_NAME)

            train_data = self.utils.load_json(json_file_path=train_annotations_local_path)
            test_data = self.utils.load_json(json_file_path=test_annotations_local_path)
            logger.info("Loaded train data and test data")

            prepared_train_data_local_path = os.path.join(ARTIFACTS_DIR, MODEL_TRAINER_ARTIFACTS_DIR, SPACY_TRAIN_DATA_FORMAT_NAME)
            prepared_test_data_local_path = os.path.join(ARTIFACTS_DIR, MODEL_TRAINER_ARTIFACTS_DIR, SPACY_TEST_DATA_FORMAT_NAME)

            self.create_spacy_format_data(data=train_data, docbin_obj=db, output_file_path=prepared_train_data_local_path, nlp_model=nlp)
            self.create_spacy_format_data(data=test_data, docbin_obj=db, output_file_path=prepared_test_data_local_path, nlp_model=nlp)
            ...
            os.system(f'python -m spacy init config {config_file_path} --lang en --pipeline ner --optimize efficiency')
